...
final Create2Factory =
    EthereumAddress.fromHex('0xce0042B868300000d44A59004Da54A005ffdcf9f');
...
class EIP4337Lib {
  ...
  static String getWalletCodeWithArgs(
      IContract contract, List<dynamic> initArgs) {
    var initCodeWithArgs = contract.bytecode;
    final walletConstructor =
        contract.ABI.functions.singleWhere((f) => f.isConstructor);
    final initParamsEncoded =
        bytesToHex(walletConstructor.encodeCall(initArgs), include0x: false);
    initCodeWithArgs += initParamsEncoded;
    // LogUtil.d('initCodeWithArgs: $initCodeWithArgs');
    return initCodeWithArgs;
  }

  static String calculateWalletAddressByCode(
      IContract contract, List<dynamic> initArgs, BigInt salt) {
  ...
  static String calculateWalletAddressByCodeHash(
      Uint8List initCodeHash, BigInt salt) {
    final saltBytes32 = bytesToHex(unsignedIntToBytes(salt),
        forcePadLength: 64, include0x: true, padToEvenLength: true);
    // LogUtil.d('saltBytes32: $saltBytes32');
    return getCreate2Address(Create2Factory, saltBytes32, initCodeHash);
  }

  // create2
  static String getCreate2Address(
      EthereumAddress factory, String saltBytes32, Uint8List initCodeHash) {
    final salt = hexToBytes(saltBytes32);
    // LogUtil.d('getCreate2Address: len ${salt.length}, ${initCodeHash.length}');
    final input = '0xff' +
        factory.hexNo0x +
        bytesToHex(salt, include0x: false) +
        bytesToHex(initCodeHash, include0x: false);
    return bytesToHex(keccak256(hexToBytes(input)).sublist(12),
        include0x: true);
  }
  ...
  static UserOperation activateWalletOp(
      ...
      BigInt salt) {
    final initCodeWithArgs = getWalletCodeWithArgs(
      SoulWallet(),
      [entryPointAddress, ownerAddress, tokenAddress, paymasterAddress],
    );
    final initCodeHash = keccak256(hexToBytes(initCodeWithArgs));
    ...
    userOperation.verificationGas =
        BigInt.from(100000 + 3200 + 400 * userOperation.initCode.length + 400);
  ...
  static Future<BigInt> getNonce(
    ...
    if (code.isNotEmpty) {
      final SoulWalletContract =
          DeployedContract(SoulWallet().ABI, walletAddress);
      final getNonce = SoulWalletContract.function("nonce");
      final response = await web3
          .call(contract: SoulWalletContract, function: getNonce, params: []);
      // LogUtil.d(response);
