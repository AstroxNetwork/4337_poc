...
class VerifyingPaymaster extends _i1.GeneratedContract {
  ...
  /// set by the contract.
  Future<String> addStake(BigInt extraUnstakeDelaySec,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[1];
    assert(checkSignature(function, '0396cb60'));
    final params = [extraUnstakeDelaySec];
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<_i2.Uint8List> getHash(dynamic userOp, {_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[5];
    assert(checkSignature(function, '813b3839'));
    final params = [userOp];
    final response = await read(function, params, atBlock);
  ...
  Future<String> postOp(
      BigInt mode, _i2.Uint8List context, BigInt actualGasCost,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[7];
    assert(checkSignature(function, 'a9a23409'));
    final params = [mode, context, actualGasCost];
    return write(credentials, transaction, function, params);
  }
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> setEntryPoint(_i1.EthereumAddress _entryPoint,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[9];
    assert(checkSignature(function, '584465f2'));
    final params = [_entryPoint];
    return write(credentials, transaction, function, params);
  }
  ...
  Future<_i2.Uint8List> validatePaymasterUserOp(
      dynamic userOp, _i2.Uint8List $param8, BigInt requiredPreFund,
      {_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[12];
    assert(checkSignature(function, '7bfd0845'));
    final params = [userOp, $param8, requiredPreFund];
    final response = await read(function, params, atBlock);
    return (response[0] as _i2.Uint8List);
  }
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<_i1.EthereumAddress> verifyingSigner({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[13];
    assert(checkSignature(function, '23d9ac9b'));
    final params = [];
    final response = await read(function, params, atBlock);
    return (response[0] as _i1.EthereumAddress);
  }
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> withdrawTo(_i1.EthereumAddress withdrawAddress, BigInt amount,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
